#!/usr/bin/env bash
# CLAD survival — v7 single-task (task=surv, surv_endpoint=clad)
# Reuses ACR Phase-1 encoder weights from results_mm_abmil_v7_alt_p1.
# TTE: clad_days (biopsy→CLAD); post-CLAD biopsies excluded; censored = study_end−anchor_dt.
set -euo pipefail

TRAIN="/home/aih/dinesh.haridoss/chicago_mil/train_mm_abmil_v7.py"
SAVE_DIR="/lustre/groups/aih/dinesh.haridoss/home_backup/chicago_mil/results_mm_abmil_v7_clad"
...
echo "===== Submitting CLAD survival (v7) ====="
...
for FOLD in $FOLDS; do
  for GROUP in $VARIANTS; do
    ...
    JOB="clad_v7_s${SPLIT}f${FOLD}_${GROUP}"
...
echo "==== CLAD v7 | ${GROUP} | split=${SPLIT} fold=${FOLD} | \$(date) ===="
...
echo "==== Done CLAD v7 | ${GROUP} | split=${SPLIT} fold=${FOLD} | \$(date) ===="
SBATCH
