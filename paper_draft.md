...
To ask what the models attend to, we performed population-level attribution on the best model for each task, tracing PMA seed vectors back through their patch attention to pre-computed morphological clusters, then contrasting seed content between the highest- and lowest-risk patient tertiles and testing reproducibility across all five outer splits (Fig. 3). The death model, our strongest and most stable, yielded a signature reproducible in every split and consistent across two independent modalities — H&E and CT — a finding we consider the strongest biological result of this study.

In **H&E histology**, seeds attending to clusters characterised by acute alveolar inflammation, haemorrhage and preserved parenchymal architecture (clusters 0–21) were robustly enriched in the *low-risk* (survivor) group. The direction is initially counterintuitive but clinically transparent. Patients who survive to undergo repeated surveillance biopsies tend to experience acute, treatable inflammatory episodes on a substrate of intact gas-exchange architecture — the microscopic correlate of maintained functional reserve. The inflammatory infiltrate they exhibit is consistent with manageable ACR: perivascular lymphocytic cuffing, endothelialitis and alveolitis that respond to corticosteroids and intensified immunosuppression [3,7]. This alveolar preservation and active but treatable inflammation is precisely the substrate of survivable lung allograft disease.

In **CT imaging**, a concentrated set of clusters (C0–C2) was reproducibly enriched in the *high-risk* group across every split. These correspond to patterns of global structural deterioration — parenchymal loss, air trapping, mosaic attenuation and volume reduction — that a single transbronchial biopsy, sampling one airway, cannot capture. CT integrates architectural decline across the whole lung; in the context of CLAD, these changes correspond to the radiological correlates of early BOS (air trapping, mosaic attenuation) and RAS (pleuroparenchymal fibroelastosis, volume loss) [21]. That CT-based structural deterioration was the dominant high-risk signal while H&E inflammation was the dominant low-risk signal is a mechanistic bifurcation: treatable local inflammation versus diffuse irreversible structural remodelling of the whole organ.

The two modalities are therefore complementary by construction. Histology reports the microenvironment at one sampled site; CT integrates architectural decline across the whole lung. Their concordant contribution — protective local inflammation, harmful global structural loss — underwrites the death model's accuracy and provides a coherent mechanistic narrative. Patients with active but alveolus-preserving inflammation on biopsy and no diffuse CT deterioration represent a treatable acute phase; those with diffuse CT structural change represent pre-clinical end-stage disease, regardless of biopsy appearance.

The ACR-survival model echoed this picture. CT seeds were again enriched in the high-risk group (reproducible in four of five splits), and the learned biopsy weighting concentrated on early visits, together indicating that the early-established structural quality of the allograft predicts the long-term rejection trajectory. Attribution for CLAD and for ACR classification was inconsistent across splits, reflecting smaller effect sizes, greater outcome heterogeneity (particularly the conflation of BOS and RAS in the CLAD label), and the limited power of a ~350-patient single-centre cohort for these noisier endpoints.

**Fig. 3a — Death survival: population-level seed attribution (split 2, best model)**
...
**Fig. 3d — Cross-split mean ± s.d. seed attribution — ACR survival (5 splits)**

![ACR surv agg](figures/interpretability/agg/Lpop_K_agg_acr_surv.png)

### H&E histology dominates ACR classification when available; modality integration drives survival

To disentangle the contribution of each modality, we analysed the per-task modality gate values (SetMIL-MT variants) and performed unimodal ablations in which the trained multimodal model is evaluated with a single modality present (Fig. 4E, 4F). For ACR classification, the unimodal ablation on split 2 reveals a striking result: when H&E is the sole modality (n = 113 test observations), the model achieves BACC = 0.865, substantially above the full-modality test-set performance (0.680 for this split). This is not a contradiction — H&E is the gold-standard basis for ACR grading in clinical practice [3], and the model has learned to exploit this histological signal faithfully. The discrepancy between unimodal (0.865) and full-population (0.623 mean BACC) performance reflects the high rate of missing H&E at the visit level: BAL cytology (n = 16, BACC = 0.200), CT and clinical data cannot compensate for absent histology on this locally determined endpoint.

For survival endpoints, by contrast, the modality gate analysis reveals that clinical features and CT contribute substantially, particularly for CLAD (where spirometry, FEV1 trajectory and laboratory trends encode the canonical CLAD phenotype) and for death (where CT structural patterns are the dominant high-risk signal). The learned gate allows each task to weight modalities independently, avoiding the pitfall of cross-modal noise contamination identified in the SAB ablation (below).

The cross-modal interaction (SAB) ablation is also informative. Removing SAB improved ACR classification (BACC 0.623 versus 0.595 with SAB) but degraded CLAD survival (0.563 with SAB versus 0.536 without). Cross-modal mixing helps when an endpoint genuinely integrates signals across modalities — CLAD, a whole-organ process measured by spirometry, CT and BAL — and hurts when a single modality carries most of the signal and cross-talk injects noise — ACR grade, read primarily from histology. This argues against one-size-fits-all fusion.
...
**Fig. 7 — Per-patient longitudinal risk trajectories (Longitudinal-MK, death and ACR-survival, representative patients)**

*Top: Predicted risk score (y-axis) at each biopsy visit (x-axis, days post-transplant) for four representative patients — one stable long-term survivor, one patient developing CLAD, one acute ACR responder and one non-survivor. Coloured background bands correspond to biopsy-weight magnitude (warm = high weight, cool = low weight). Vertical dashed lines mark clinical events (ACR episode, CLAD onset, death). Bottom: learned biopsy-weight profile for the prediction anchored at the last available visit, showing which prior visits dominated the final prediction for each patient.*

*(Figure to be generated by running the per-patient trajectory analysis on the split-2 best model.)*
...
### Supplementary Note 2 | Unimodal ablation (split 2, ACR classification, SetMIL-MT no SAB)

When evaluated with a single modality present, the trained multimodal model achieves the following performance on the split-2 test fold:

| Modality | n (test obs.) | BACC | AUC |
|---|---|---|---|
| H&E | 113 | **0.865** | 0.957 |
| CT | 137 | 0.556 | 0.634 |
| Clinical | 262 | 0.503 | 0.597 |
| BAL | 16 | 0.200 | 0.291 |

H&E alone, when present, achieves near-perfect separation for the ACR classification task — consistent with H&E being the clinical gold standard for ACR grading. The discrepancy between H&E unimodal performance (0.865) and full-test-set performance (mean BACC 0.623) reflects the high rate of missing H&E and the poor performance of the remaining modalities on this locally determined endpoint. BAL (n = 16, BACC = 0.200) is likely an artefact of the extreme class imbalance in the BAL-available subset rather than a genuine negative result.
