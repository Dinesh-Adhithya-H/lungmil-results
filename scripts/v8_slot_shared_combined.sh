#!/usr/bin/env bash
# v8_slot_shared_combined.sh — SharedSlotMIL final training on train+val combined
#
# Prerequisites: all 4 per-fold HP sweeps must be complete
#   (results/mm_abmil_v8/phase2/split1_fold{0..3}/slot_mega_shared/hp_sweep/hp_sweep_p2.json)
#
# Selects best HP globally across all 4 folds, then trains fold 0 on
# train+val combined (all folds share the same test set).
#
# Usage:
#   sbatch scripts/v8_slot_shared_combined.sh
#
#SBATCH --job-name=slot_combined
...
#SBATCH --output=/home/aih/dinesh.haridoss/chicago_mil/results_mm_abmil_v8/slurm_logs/%j_slot_combined.out
#SBATCH --error=/home/aih/dinesh.haridoss/chicago_mil/results_mm_abmil_v8/slurm_logs/%j_slot_combined.err
...
SAVE_DIR="${OUT_DIR}/phase2/split${SPLIT}_fold${FOLD}/slot_mega_shared_combined"
...
echo "==============================="
echo " slot_combined  fold=${FOLD}  job=${SLURM_JOB_ID}"
...
python3 -u "${HOME_MIL}/train_mm_abmil_v8.py" \
    ...
    --combined-train                 \
    --alternating                    \
    --p2-tag         shared_combined \
    --hp-source-tag  shared          \
    --modal-dropout  0.3             \
